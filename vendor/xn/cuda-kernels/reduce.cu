...
__device__ void layernorm(const T *x, T *dst, const T *alpha, const T *beta,
                          const int ncols, const int block_size,
                          const float eps, const int remove_mean) {
  ...
  if (alpha == nullptr && beta == nullptr) {
    for (int col = tid; col < ncols; col += block_size) {
      float lhs =
          (static_cast<float>(x[row * ncols + col]) - mean_offset) * inv_std;
...
template <typename T>
__device__ void rmsnorm(const T *x, T *dst, const T *alpha, const int ncols,
  ...
  if (alpha == nullptr) {
    for (int col = tid; col < ncols; col += block_size) {
      dst[row * ncols + col] =
          static_cast<T>(scale * static_cast<float>(x[row * ncols + col]));
    }
  } else {
    for (int col = tid; col < ncols; col += block_size) {
      float a = static_cast<float>(alpha[col]);
      dst[row * ncols + col] =
          static_cast<T>(scale * static_cast<float>(x[row * ncols + col]) * a);
    }
  }
}
...
template <typename T>
__device__ void ropei(const T *src, const T *cos, const T *sin, T *dst,
                      const uint32_t bh, const uint32_t td,
...
template <typename T>
__device__ void rope(const T *src, const T *cos, const T *sin, T *dst,
                     const uint32_t bh, const uint32_t td, const uint32_t d,
                     const uint32_t stride_b) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (2 * idx >= bh * td)
    return;
...
template <typename T>
__device__ void rope_thd(const T *src, const T *cos, const T *sin, T *dst,
                         const uint32_t b, const uint32_t t, const uint32_t h,
                         const uint32_t d, const uint32_t stride_b) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (2 * idx >= b * t * h * d)
    return;
...
template <typename T>
__device__ void fast_argmin(const size_t src_numel,
                            const size_t el_to_sum_per_block,
                            const size_t num_dims, const size_t *info,
                            const T *src, int64_t *dst) {
  const size_t *dims = info;
  const size_t *strides = info + num_dims;
  ...
  while (idx < stop_idx) {
    ...
    if (not_set || src[strided_i] < shr[tid]) {
      shr[tid] = src[strided_i];
      // Assume that the reduction takes place over the last dimension which is
      // contiguous.
...
template <typename T>
__device__ void fast_argmax(const size_t src_numel,
                            const size_t el_to_sum_per_block,
                            const size_t num_dims, const size_t *info,
                            const T *src, int64_t *dst) {
  const size_t *dims = info;
...
#define FAST_OP(TYPENAME, MIN_NAME, MAX_NAME, ARGMIN_NAME, ARGMAX_NAME,        \
                SUM_NAME)                                                      \
...
#define RMSNORM_OP(TYPENAME, FN_NAME)                                          \
  extern "C" __global__ void FN_NAME(const TYPENAME *src, TYPENAME *dst,       \
                                     const TYPENAME *alpha, const int n_cols,  \
                                     const int block_size, const float eps) {  \
...
#define LAYERNORM_OP(TYPENAME, FN_NAME)                                        \
  extern "C" __global__ void FN_NAME(                                          \
      ...
      const TYPENAME *beta, const int n_cols, const int block_size,            \
      const float eps, const int remove_mean) {                                \
...
#define ROPE_OP(TYPENAME, FN_NAME, FN_NAME_I, FN_NAME_THD)                     \
  extern "C" __global__ void FN_NAME_I(                                        \
      const TYPENAME *src, const TYPENAME *cos, const TYPENAME *sin,           \
      TYPENAME *dst, const uint32_t bh, const uint32_t td,                     \
      const uint32_t stride_b) {                                               \
    ropei<TYPENAME>(src, cos, sin, dst, bh, td, stride_b);                     \
  }                                                                            \
  extern "C" __global__ void FN_NAME(                                          \
      const TYPENAME *src, const TYPENAME *cos, const TYPENAME *sin,           \
      TYPENAME *dst, const uint32_t bh, const uint32_t td, const uint32_t d,   \
      const uint32_t stride_b) {                                               \
    rope<TYPENAME>(src, cos, sin, dst, bh, td, d, stride_b);                   \
  }                                                                            \
  extern "C" __global__ void FN_NAME_THD(                                      \
      const TYPENAME *src, const TYPENAME *cos, const TYPENAME *sin,           \
      TYPENAME *dst, const uint32_t b, const uint32_t t, const uint32_t h,     \
      const uint32_t d, const uint32_t stride_b) {                             \
...
FAST_OP(__nv_bfloat16, fast_min_bf16, fast_max_bf16, fast_argmin_bf16,
        fast_argmax_bf16, fast_sum_bf16)
#endif
...
FAST_OP(__half, fast_min_f16, fast_max_f16, fast_argmin_f16, fast_argmax_f16,
        fast_sum_f16)
#endif
...
FAST_OP(float, fast_min_f32, fast_max_f32, fast_argmin_f32, fast_argmax_f32,
        fast_sum_f32)
FAST_OP(double, fast_min_f64, fast_max_f64, fast_argmin_f64, fast_argmax_f64,
        fast_sum_f64)
FAST_OP(uint32_t, fast_min_u32, fast_max_u32, fast_argmin_u32, fast_argmax_u32,
        fast_sum_u32)
FAST_OP(int64_t, fast_min_i64, fast_max_i64, fast_argmin_i64, fast_argmax_i64,
        fast_sum_i64)
FAST_OP(uint8_t, fast_min_u8, fast_max_u8, fast_argmin_u8, fast_argmax_u8,
        fast_sum_u8)
