...
async fn handle_socket(socket: WebSocket, app: AppState) {
    ...
    if let Err(e) = result {
        tracing::warn!(error = %e, "ws session terminated");
    }
}
    ...
    B: xn::Backend + 'static,
{
    ...
    let (reply_tx, mut reply_rx) = mpsc::channel::<TtsReply>(REPLY_BUFFER);
    ...
    let outcome = run_session(app, &mut stream, &reply_tx).await;
    drop(reply_tx);
    ...
    B: xn::Backend + 'static,
{
    ...
    if let Err(e) = app.model.prompt_audio(&mut base_state, voice_emb_t) {
        send_error(reply_tx, error_codes::INTERNAL, format!("prompt_audio failed: {e}")).await?;
        return Ok(None);
    }
    ...
    B: xn::Backend + 'static,
{
    ...
    let (audio_tx, mut audio_rx) = mpsc::channel::<Vec<f32>>(AUDIO_BUFFER);
    ...
    while let Some(pcm) = audio_rx.recv().await {
        ...
        samples_emitted += n;
        if reply_tx.send(TtsReply::Audio { audio, start_s, stop_s, stream_id }).await.is_err() {
            break;
        }
    }
    drop(audio_rx);
